...
describe User do
  describe '.log_in' do
    ...
    it 'should return nil if password does not match email from entry in user table' do
      User.create(email: 'test1@gmail.com', password: 'password1')
      expect(User.log_in('test1@gmail.com', 'wrong_password1')).to eq nil
    end
    it 'should return nil if email does not match an entry in user table' do
      expect(User.log_in('test2@gmail.com', 'irrelevant_password2')).to eq nil
    end
  end
end
